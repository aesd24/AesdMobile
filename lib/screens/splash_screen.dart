...
class _SplashScreenState extends State<SplashScreen> {
  final Future<StorageAuthTokenSession> _futureAuthToken = StorageAuthTokenSession.getFormSecureStorage();
    ...
    if (await checkConnectionState()){
      _futureAuthToken.then((StorageAuthTokenSession authToken) => {
        Provider.of<Auth>(context, listen: false).setToken(
          type: authToken.type, token: authToken.token
        )
      });
      ...
      Future.delayed(
        ...
        () {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => logged ? const HomeScreen() : const LoginPage())
          );
        }
      );
    }
      ...
      showDialog(
        ...
        builder: (context){
          return AlertDialog(
            ...
            content: const Text("Connexion impossible. Vérifiez votre connexion internet et rééssayez !"),
            actions: [
              TextButton(
                onPressed: (){
                ...
                style: ButtonStyle(
                  ...
                  overlayColor: WidgetStateProperty.all(Colors.red.shade200)
                ),
